...
def prep_fit(X,scale):
    return ColumnTransformer([('num',Pipeline([('imp',SimpleImputer(strategy='median')),*([('sc',StandardScaler())] if scale else [])]),num),('cat',Pipeline([('imp',SimpleImputer(strategy='most_frequent')),('oh',OneHotEncoder(handle_unknown='ignore',sparse_output=False))]),cat)])
...
def cv_scores(kind,param):
    ...
    for a,b in cv.split(d.iloc[tr],y.iloc[tr]):
        ...
        pf=prep_fit(rawtr,kind=='knn'); X=pf.fit_transform(rawtr); V=pf.transform(rawv); X,yt=balance(X,yt,rng)
